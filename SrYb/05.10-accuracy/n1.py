...
def main():
    ...
    ns = np.arange(6)*2+24
    filenames = ['SrYb_n1_'+str(n)+'_n35_4_j2.out' for n in ns]
    dataset = get_dataset(filenames, path)
    dataset.sort(key=get_dipole)
#    show_spectrum(dataset)
#    show_one_energy_level(dataset, 0)
#    show_one_energy_level_change(dataset, 8)
    show_one_energy_level_change_together(dataset, 7)
